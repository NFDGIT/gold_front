            ...
            <van-list>
                <van-cell v-for="item in list" :key="item">
                 <van-row>
                    ...
                    <van-row>
                        <van-col span="12" class="txt_blod" style="text-align:right">
                            <van-button round type="primary" @click="rollbackSplitSetlleClick(item)"
...
import { getSplitOrder,settleSplitOrderRollback } from '../../service/order.js'
...
export default {
    ...
    created() {
        ...
        this.getgetSplitOrder()
    },
    methods: {
        getgetSplitOrder() {
            getSplitOrder(this.query).then(res => {
                this.list = res.data
            })
        },
        closed(isGetData) {
        ...
        rollbackSplitSetlleClick(order) {
             settleSplitOrderRollback({
                id: order.id,
            }).then(() => {
                Toast.success('撤销成功');
                this.closed(true)
